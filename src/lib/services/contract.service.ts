...
export class ContractService {
  ...
  private async startDepositListener() {
    const contractAddress = process.env.CONFIDEX_CONTRACT
  ...
  getUserBalance(userAddress: string, token: string): bigint {
    ...
    console.debug(
      `Retrieved balance for user ${userAddress}, token ${token}: ${balance.toString()}`
    )
    return balance
  }
  ...
  async withdrawTokensWithSignature(
    userAddress: string,
    token: string
    ...
    try {
      ...
      const contractAddress = process.env.CONFIDEX_CONTRACT as `0x${string}`
